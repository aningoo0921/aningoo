...
export default function FunctionComponent() {
    ...
    const name = 'layla';
    const show = true;
    const textStyle = { backgroundColor: 'pink', color: 'navy', fontWeight: 900 }
  ...
  return (
    ...
    1. 하나의 최상위 부모 요소로 감싸서 return
      -virtual DOM에서 컴포넌트 변화를 감지할 때
       효율적으로 비교할 수 있도록 컴포넌튼 내부에는
       하나의 DOM 트리 구조로 이루어져야 한다고 React 정함
      -div, section, article, header, footer 등
       최상위 부요 요소는 어던 요소이든 상관 없음
      -불필요한 태그를 만들지 않기 위해 <></> 태그로 감싸는 경우도 많음
      (fragment 단축 문법) -> react 
    */}
    <div>함수 컴포넌트 1</div>
    <div>함수 컴포넌트 2</div>
    {/*2-1.js 문법 사용 가능(변수) */}
    ...
    {/* 3.inline style 적용 방법 => style 속성으로 객체 전달 */}
    ...
    <button 
      ...
      <input></input>
      <input />
      {/* 절대경로는 public 폴더에서 가져옴 */}
      <img src='/logo192.png' alt='리엑트 이미지' />
      {/* src 내부의 이미지를 사용할 경우 해당 이미지  */}
