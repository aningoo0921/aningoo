...
// 하나의 요소에 여러개 이벤트 등록 가능
// mouseover는 마우스가 해당 요소 위에 올라갔을 때 실행 -> 마우스 때도 계속 적용
// hover는 마우스가 해당 요소 위에 올라가 있을 때만 적용 -> 마우스 때면 원래대로 돌아감
btn1.addEventListener('mouseover', function () {
    // 선택된 요소 자신을 수정하고 싶은 경우 this 키워드 사용
    this.style.backgroundColor = 'red';
})

//btn-2 : 화살표 함수 사용해 container에 요소 추가
...
// btn-3 : 함수 선언헤서 이벤트 등록
btn3.addEventListener('click', changeColor);
// 같은 기능을 재사용 하는 경우에 이렇게 함수를 선언하고
// addEventListener 에서 불러와 사용
...
function changeColor () {
    ...
    if (divs.length !== 0) {
      ...
      // 마지막 요소만 배경색 파란색 변경
      divs[divs.length - 1].style.backgroundColor = 'blue'; // 가장 마지막 요소가 선택이 됨
    } else {
        alert('버튼 2를 클릭해 div를 추가해주세요!')
    }
}

// btn-4 : 이벤트 적용된 자기 자신을 선택하는 this 키워드 사용
...
// 매개변수 event
// scroll 이밴트는 브라우저 전체에 대해 실행되는 동작이기 때문ㅇ[
