...
//만약 조건이 false이라면 해당 코드 실행되지 않고 넘어감
if (5 > 7) {
...
// 위의 조건을 통과하고 다음 조건문을 실행하는 경우 위의 조건들이 보두 false였음을 잊지 않기
// if (score > 100) {
//     // 100점보다 큰 점수는 없음
...
// JS DOC
/**
 * 입력받을 아이디와 비밀번호가 맞는지 확인 후 적절한 알림창 띄움
 */
function loginUser () {
...
/*
    1.변수 x의 값과 안과 첫번째 case문으 값 3을 비교
    2.두 값이 동일하다면 : (콜론) 뒤의 코드가 실행
    3.break문을 통해 해당 스코프에서 빠져나옴
    4.만약 첫번째 케이이스문의 값과 x가 다르다면 케이스 문의 값과 비교
    5.모든 케이스문의 값과 x의 값이 다르다면 default문 다음의 코드가 실행
 */
//break는 필수 작성
// default문 생략 가능
let x = 3;
...
//케이스에도 조건이 아닌 다른 값을 작성
switch (x) {
    case 3:
        console.log('x 는 3입니다.');
        break;
    case 4: 
        console.log('x는 4입니다.');
        //break 작성하지 않음 case4를 실행하고 싶을 때 case5까지 모두 실행됨
        // 해당 스코프를 빠져나가지 못하는 것
        // 원하는 코드가 실행된 이후 꼭 break 사용해 빠져나올 수 있도록 작성
        break;
    case 5:
        console.log('x는 5입니다.');
        break;
...
// 삼항연산자
//조건이 ? 조건이 참일 때 실행될 코드 : 조건이 거짓일 때 실행될 코드
